...
function rankAndExplain(jobs) {
  console.log(`🏆 Ranking top ${TOP_N} from ${jobs.length} candidates...`);

  const SKILL_KEYWORDS = ['node', 'express', 'postgres', 'javascript', 'rest', 'api', 'backend', 'fullstack', 'full-stack', 'typescript'];
  const TITLE_KEYWORDS = ['backend', 'full stack', 'fullstack', 'software engineer', 'node', 'javascript'];
  const INTERN_KEYWORDS = ['intern', 'fresher', 'trainee', 'junior', 'entry'];

  function explain(job) {
    const haystack = [job.title, job.description, ...(job.tags||[])].join(' ').toLowerCase();
    const matchedSkills = SKILL_KEYWORDS.filter(s => haystack.includes(s));
    const isIntern = INTERN_KEYWORDS.some(k => job.title.toLowerCase().includes(k));
    const locNote = job.location.toLowerCase().includes('remote') ? 'remote-friendly' : `based in ${job.location}`;
    const skillNote = matchedSkills.length > 0 ? matchedSkills.slice(0,3).join(', ') : 'your tech stack';
    const roleType = isIntern ? 'internship' : 'role';
    return `This ${roleType} at ${job.company} is ${locNote} and aligns with your ${skillNote} skills. It matches your target profile as a ${job.title.trim()} position.`;
  }
...
Actor.main(async () => {
  ...
  const picks = rankAndExplain(candidates);
